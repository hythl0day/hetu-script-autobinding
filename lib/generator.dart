...
Future<List<FileSystemEntity>> dirContents(Directory dir, List<String> ignores, List<String> whitelist) {
...
Future<List<FileDefine>> parseDartFiles(String? jsonPath, List<String> ignores) async {
  //文件读取完后将父类赋值给子类
...
void parseBegin(List<String> userPaths, String? flutterPath, List<String> packagePaths, String exportPath,
  ...
  if (files.isEmpty) {
    ...
    for (var p in fileDefines) {
      var b = await generateWrappers(p, exportPath, scriptExportPath, library: ExportType.UserDefine);
  ...
  for (var a in packagePaths) {
    ...
    if (files.isNotEmpty) {
      ...
      for (var p in fileDefines) {
        var b =
            await generateWrappers(p, exportPath, scriptExportPath, library: ExportType.Package, libName: packageName);
        customImportMap[p.filePath] = 'package:$packageName/$packageName.dart';
        if (b.isNotEmpty) {
          allBindings.addAll(b);
          fileEntries
              .add({'import_file_name': 'package://$packageName/${path.basenameWithoutExtension(p.filePath)}.ht'});
        }
      }
  ...
  renderTemplate('template/ht_script_binding.mustache', userTemplateVars, '$exportPath/ht_script_binding.dart');
  ...
  if (flutterPath != null) {
    ...
    for (var p in dartDefines) {
      var libName = path.split(path.relative(p.filePath, from: dartSourceRoot)).first;
      packages[libName] ??= [];
      var fileEntries = packages[libName];

      var b =
          await generateWrappers(p, exportPath, scriptExportPath, library: ExportType.DartLibrary, libName: libName);

      if (b.isNotEmpty) {
        allBindings.addAll(b);
        fileEntries?.add({'import_file_name': 'dart://$libName/${path.basenameWithoutExtension(p.filePath)}.ht'});
      }
    }
    packages.forEach((key, value) {
      if (value.isNotEmpty) {
        var templateVars = {'import_files': value};
        renderTemplate('template/import_entry.mustache', templateVars, '$scriptExportPath/dart/$key.ht');
      }
    });
    packages.clear();
    for (var p in flutterDefines) {
      var libName = path.split(path.relative(p.filePath, from: flutterSourceRoot)).first;
      packages[libName] ??= [];
      var fileEntries = packages[libName];

      var b =
          await generateWrappers(p, exportPath, scriptExportPath, library: ExportType.FlutterLibrary, libName: libName);

      if (b.isNotEmpty) {
        allBindings.addAll(b);
        fileEntries?.add({'import_file_name': 'flutter://$libName/${path.basenameWithoutExtension(p.filePath)}.ht'});
      }
    }
    packages.forEach((key, value) {
      if (value.isNotEmpty) {
        var templateVars = {'import_files': value};
        renderTemplate('template/import_entry.mustache', templateVars, '$scriptExportPath/flutter/$key.ht');
      }
    });
    ...
    renderTemplate(
        'template/ht_library_script_binding.mustache', libTemplateVars, '$exportPath/ht_library_script_binding.dart');
  }

  //检查是否有缺失的执行文件（库绑定、自定义绑定）
  if (await FileSystemEntity.type('$exportPath/ht_script_binding.dart') == FileSystemEntityType.notFound) {
    var userTemplateVars = {'user_api_import': [], 'user_bindings': [], 'ht_user_bindings': []};
    renderTemplate('template/ht_script_binding.mustache', userTemplateVars, '$exportPath/ht_script_binding.dart');
  }
